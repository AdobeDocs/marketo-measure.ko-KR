...
1. **+** 아이콘을 클릭하여 규칙 만들기를 시작합니다.
   ...
   ![규칙 추가 단추가 있는 Campaign 동기화 설정 페이지](assets/1-1.png)
...
1. [!UICONTROL Campaign] 또는 [!UICONTROL Campaign Member] 필드에서 규칙을 만들 수 있습니다. 유효성 검사가 예상되는 연산자 및 값으로 규칙의 나머지 부분을 채웁니다. 아래 예에서는 이름으로 특정 Campaign을 확인하고 있습니다.
   ...
   ![Campaign 필드 선택 및 연산자 옵션이 있는 규칙 빌더](assets/2-1.png)
...
1. 접점 날짜를 선택합니다. 중괄호 `{`을(를) 입력한 후 가능한 날짜 목록이 나타납니다. 그러면 규칙에서 만든 모든 터치포인트에 적용할 날짜를 선택할 수 있습니다.
   ...
   ![사용 가능한 날짜 필드를 표시하는 자동 완성 드롭다운이 있는 터치포인트 날짜 필드](assets/3-1.png)
...
1. 확인 표시를 클릭한 다음 필요에 따라 추가 캠페인에 대한 규칙을 추가합니다.
   ...
   ![더 많은 규칙을 추가할 수 있는 확인 표시 및 옵션이 있는 완료된 규칙](assets/4-1.png)
   ...
   ![CRM 동기화 유형 구성을 표시하는 여러 캠페인 동기화 규칙](assets/5-1.png)
...
![동적 터치포인트 날짜 필드를 보여주는 Buyer Attribution Touchpoint 레코드](assets/6-1.png)
