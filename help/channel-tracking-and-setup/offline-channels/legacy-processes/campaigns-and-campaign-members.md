...
![구매자 접점 사용 필드를 보여주는 Salesforce Campaign 페이지 레이아웃](assets/1.png)

선택 목록에서 사용할 수 있는 옵션은 다음과 같습니다.

![구매자 터치포인트 선택 목록 드롭다운을 캠페인 멤버 포함 옵션으로 사용](assets/2.png)
...
![접점 시작 날짜 및 접점 종료 날짜 필드를 표시하는 캠페인 레코드](assets/3.png)
...
![잠재 고객, 연락처, 영업 기회 및 상태 날짜에 대한 접점 상태 필드를 표시하는 캠페인 멤버 레코드](assets/4.png)
...
![날짜 재정의에 대한 Buyer Touchpoint 날짜 필드가 있는 캠페인 멤버 레코드](assets/5.png)

Buyer Touchpoint 날짜를 사용해야 하는지 여부를 확인하려면 캠페인에 대해 선택한 [!DNL Marketo Measure]에 따라 [!UICONTROL Sync Type]에 의해 날짜가 결정되는 방법을 알아보세요.
...
![터치포인트 날짜 일괄 업데이트 단추가 있는 캠페인 페이지 레이아웃](assets/6.png)
...
null Buyer Touchpoint 날짜에 대해 지원되지 않는 검색을 표시하는 ![대량 업데이트 인터페이스](assets/7.png)
...
![일정 선택기로 터치포인트 날짜 인터페이스 일괄 업데이트 및 선택한 레코드 업데이트 단추](assets/8.png)

## 캠페인 비용 {#campaign-costs}

이 문서[에서 캠페인 비용 &#x200B;](/help/marketing-spend/crm-campaign-costs.md){target="_blank"}에 대해 모두 알아보세요.
...
>[[!DNL Marketo Measure] 자습서: 오프라인 채널 매핑](https://experienceleague.adobe.com/ko/docs/marketo-measure-learn/tutorials/onboarding/marketo-measure-salesforce/mapping-offline-channels){target="_blank"}
