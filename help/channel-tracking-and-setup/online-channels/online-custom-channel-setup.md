...
![원본 템플릿 다운로드 및 현재 규칙 다운로드 옵션을 사용하여 온라인 채널 구성을 표시하는 설정 페이지](assets/1.png)

스프레드시트에는 7개의 열이 있습니다.

![채널, 하위 채널, 캠페인, Medium, Source, 랜딩 페이지 및 참조 웹 사이트의 7개 열을 표시하는 사용자 지정 채널 스프레드시트](assets/2.png)

* **채널:** 여기에 다양한 마케팅 채널 추가
* **하위 채널:** 여기에 해당 하위 채널을 추가합니다.
* **Campaign:** 여기에 캠페인 이름을 추가합니다. 값이 [!DNL Marketo Measure] 활동 기능에 대한 UTM에서 가져오는지 Salesforce 캠페인에서 가져오는지 여부
...
![Social.Facebook 규칙 예가 강조 표시된 하향식 우선 순위 순서를 보여주는 채널 규칙 스프레드시트](assets/3.png)
...
![사용자 지정 가능한 채널 및 하위 채널 이름이 있는 12개의 통합 플랫폼을 표시하는 기본 채널 규칙](assets/4.png)
...
![Facebook 채널 규칙 예제는 하위 채널에 매핑된 다른 매개 변수가 있는 여러 행을 보여줍니다](assets/5.png)
...
![일치하지 않는 데이터에 대해 와일드카드 별표를 모두 catch-all 버킷으로 표시하는 기타 채널 규칙](assets/6.png)
