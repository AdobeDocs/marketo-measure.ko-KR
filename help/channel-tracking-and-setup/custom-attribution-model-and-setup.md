...
1. 사용자 지정 모델에 포함할 단계를 결정합니다.
   ...
   ![최대 6개의 사용자 지정 단계가 허용됩니다(기본값 제외:](assets/custom-models-1.png)).
...
사용자 지정 모델에 사용자 지정 필드를 사용하는 경우 CRM에서 필드 기록 추적을 활성화해야 합니다. 필드 기록 추적을 사용하도록 설정하는 지침은 [사용자 지정 모델 설정: 필드 기록 추적 사용](/help/channel-tracking-and-setup/custom-model-setup-enable-field-history-tracking.md)을 참조하세요.
...
1. 사용자 지정 모델의 속성 비율을 결정합니다.
   ...
   **[!UICONTROL Attribution Settings]** 앱의 [!DNL Marketo Measure]&#x200B;(으)로 이동합니다. 사용자 지정 단계는 여기에 속성 테이블에 표시됩니다. 속성 테이블에는 모든 [!DNL Marketo Measure] 속성 모델과 각 모델의 속성 가중치가 표시됩니다. 처음 5개 모델의 기여도 분석 비율은 고정되며 변경할 수 없습니다.
   ...
   _Custom_ 열 왼쪽에는 **[!DNL Marketo Measure]기계 학습 모델**&#x200B;이 있습니다. 머신 러닝 모델은 각 사용자 지정 단계에서 발생한 사항에 따라 거래 승인에 대한 상대적 중요도에 따라 속성 가중치를 계산합니다. 기계 학습 모델에 대한 자세한 내용은 [기계 학습 모델 FAQ](/help/channel-tracking-and-setup/machine-learning-model-faq.md)를 참조하세요.

   ![사용자 지정 열의 왼쪽에 Marketo Measure이 있습니다](assets/custom-models-2.png)
...
이제 사용자 지정 모델이 활성화되어 있지 않더라도 마케팅 Funnel에서 사용자 지정 단계를 볼 수 있습니다. 이는 Funnel Stage 기능을 사용하는 것입니다. 이제 funnel 스테이지를 통해 funnel에 스테이지를 추가할 수 있지만 이에 대한 속성은 표시되지 않습니다.

Funnel 단계는 여전히 터치 포인트로 추적되며 CRM에서 터치 포인트 위치로 표시됩니다. 사용자 지정 모델이 없어도 양식 채우기가 있는 경우 이러한 터치포인트는 여전히 중간 터치 속성을 받을 수 있지만(중간 터치는 10%), 웹 방문만 있는 경우 속성 크레딧이 0입니다.
...
![아래에서 볼 수 있듯이 Diligence 단계가 별도로 포함되었습니다](assets/custom-models-7.png)
