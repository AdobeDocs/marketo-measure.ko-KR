...
ABM(Account-Based Marketing)의 등장으로 [!DNL Marketo Measure]이(가) ABM 전략을 어떻게 보완하는지 이해하는 것이 중요합니다. [!DNL Marketo Measure] 은(는) 각 리드에 대한 각 터치포인트를 표시하고 계정 아래에 연락합니다.

## [!UICONTROL What] {#the-what}

하나의 계정에 여러 개의 영업 기회가 있는 경우 서로 다른 영업 기회는 처음 두 터치포인트(FT) 및 리드 생성(LC)을 공유합니다. 새로운 Opportunity 가 생성되면 Opportunity 를 funnel 아래로 더 이동하는 데 기여한 Touchpoints에 개별 매출액이 할당됩니다. 기회와 연결된 터치포인트는 구매자 속성 터치포인트(BAT)로 간주됩니다.
