---
description: Marketo Measure 사용자를 위한 터치포인트 제거 및 터치포인트 억제 지침
title: 접점 제거 및 접점 억제
...
![다음 항목과 연결할 연산자 목록에서 선택](assets/touchpoint-settings-10.png)
...
![또는 그룹 외부에 새 문을 추가하여 &quot;OR&quot; 논리를 활용하십시오](assets/touchpoint-settings-11.png)

`3)` 규칙이 복잡해지고 그룹을 다시 만들고 각 문을 약간 변경해야 하는 경우 [!UICONTROL Clone] 옵션을 사용하여 작업을 간단하게 하십시오.

![3) 규칙이 복잡해지고 그룹을 다시 만들어야 하는 경우](assets/touchpoint-settings-6.png)

실수를 했다면 걱정하지 마세요. 명령문의 개별 행이나 전체 그룹도 삭제할 수 있습니다.

![실수를 한 경우 걱정하지 마십시오. 개별 행을 삭제할 수 있습니다](assets/touchpoint-settings-7.png)
...
![4) 원하는 경우 구매자 속성 터치포인트에 대한 규칙 설정](assets/touchpoint-settings-8.png)

완료하려면 규칙을 [!UICONTROL Save and Process]하세요. 많은 변경 작업을 수행하는 경우 그 동안 변경 내용을 저장해야 합니다. [!DNL Marketo Measure] 을 클릭할 때까지 터치포인트 제거를 실제로 시작하지 않습니다.
[!UICONTROL **저장 및 처리**].
