...
이제 [Data Warehouse](/help/data-warehouse/data-warehouse-schema.md){target="_blank"}의 URL 테이블에 웹 데이터 테이블 외에 페이지 제목 필드가 포함됩니다.
...
* [새 대시보드 검색 가이드](/help/marketo-measure-discover-ui/new-discover-dashboard-guide.md){target="_blank"}
* [대시보드 기본 사항 살펴보기](/help/marketo-measure-discover-ui/discover-dashboard-basics.md){target="_blank"}
* [수익 개요 대시보드](/help/marketo-measure-discover-ui/revenue-overview-dashboard.md){target="_blank"}
* [속성 수익 대시보드](/help/marketo-measure-discover-ui/attributed-revenue-dashboard.md){target="_blank"}
* [ROI 대시보드](/help/marketo-measure-discover-ui/roi-dashboard.md){target="_blank"}
* [Passport 대시보드](/help/marketo-measure-discover-ui/passport-dashboard.md){target="_blank"}
...
![접점이 있는 잠재 고객/연락처에 대한 Salesforce 보고서 유형 설정](assets/release-notes-2023-1.png)

* 제거된 필드를 활용하는 기존 보고서의 기능을 캡처하는 보고서를 만듭니다. 이 프로세스의 일부로 아래 지정된 대로 보고서의 필드를 변경하려고 합니다.

* 리드/연락처 FT/LC 필드 제거:

![제거할 이전 리드 및 연락처 FT/LC 필드를 강조 표시하는 보고서 열](assets/release-notes-2023-2.png)

* 접점 필드 추가:

![FT/LC 필드 대신 추가할 터치포인트 필드를 표시하는 보고서 열](assets/release-notes-2023-3.png)

* 접점 위치 필터 및 날짜 필드를 포함하여 FT/LC 필드를 사용하는 필터는 다음과 같이 업데이트해야 합니다.

![접점 위치 필터 구성 예](assets/release-notes-2023-4.png)

![날짜 필드 필터가 접점 기반 필드로 업데이트되었습니다](assets/release-notes-2023-5.png)
...
* RefreshToken을 사용하는 OAuth의 오래된 메서드는 더 이상 사용되지 않습니다. ClientSecret 사용에 대한 Microsoft의 모범 사례를 준수하도록 자격 증명을 업데이트하는 방법은 [이 안내서](/help/marketo-measure-and-dynamics/oauth-with-azure-active-directory-for-dynamics-crm.md){target="_blank"}를 참조하십시오.
