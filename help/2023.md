---
description: Marketo Measure 사용자를 위한 2023 릴리스 노트 지침
title: 2023 릴리스 정보
...
이제 새로 디자인된 [웹 트래픽 대시보드](/help/web-traffic-dashboard.md){target="_blank"}에 모든 고객이 액세스할 수 있습니다. 이 대시보드는 웹 사이트의 방문자 상호 작용에 대한 전체 개요를 제공합니다. 특정 양식 URL 또는 랜딩 페이지에서 URL당 고유 방문자 수, 전체 방문, 페이지 보기 수, 양식 제출과 같은 지표를 분석할 수 있습니다. 월별 트래픽 트렌드를 추적하고 성과가 좋은 유료 미디어를 식별할 수도 있으므로 최적의 수익 생성을 위한 전략을 구체화하는 데 도움이 됩니다.
...
데이터 개인 정보 보호 규정 준수를 보장하기 위해 장기 저장소에서 IP 주소 데이터를 제거하고 있습니다. 현재 다음 Snowflake 테이블 및 보기에는 IP 주소가 포함되어 있으며 이 데이터를 제거하고 새 지리적 위치 정보를 추가할 계획입니다.

<table style="width:400px">
<thead>
  <tr>
    <th style="width:50%">테이블</th>
    <th>보기 횟수</th>
  </tr>
</thead>
<tbody>
...
이제 [Data Warehouse](/help/data-warehouse-schema.md){target="_blank"}의 URL 테이블에 웹 데이터 테이블 외에 페이지 제목 필드가 포함됩니다.
...
* [새로운 Discover Dashboard 안내서](/help/new-discover-dashboard-guide.md){target="_blank"}
* [대시보드 기본 사항 살펴보기](/help/discover-dashboard-basics.md){target="_blank"}
* [수익 개요 대시보드](/help/revenue-overview-dashboard.md){target="_blank"}
* [속성 수익 대시보드](/help/attributed-revenue-dashboard.md){target="_blank"}
* [ROI 대시보드](/help/roi-dashboard.md){target="_blank"}
* [Passport 대시보드](/help/passport-dashboard.md){target="_blank"}
...
통합을 단순화하고 Salesforce 표준 개체로 내보낼 필요가 없도록 리드/연락처 개체로의 내보내기 작업을 단계적으로 확장할 예정입니다. 고객이 터치포인트 오브젝트에서 동일한 데이터를 얻을 수 있으므로 아래 나열된 비정규화된 필드도 더 이상 사용되지 않습니다. _&#x200B;**사용 중단 타임라인은 2024년 6월입니다.**&#x200B;_
...
![접점이 있거나 없는 잠재 고객 및 연락처에 대한 새 보고서 유형을 만듭니다.](assets/release-notes-3.png)

* 제거된 필드를 활용하는 기존 보고서의 기능을 캡처하는 보고서를 만듭니다. 이 프로세스의 일부로 아래 지정된 대로 보고서의 필드를 변경하려고 합니다.

* 리드/연락처 FT/LC 필드 제거:

![리드/연락처 FT/LC 필드 제거:](assets/release-notes-5.png)

* 접점 필드 추가:

![접점 필드 추가:](assets/release-notes-4.png)

* 접점 위치 필터 및 날짜 필드를 포함하여 FT/LC 필드를 사용하는 필터는 다음과 같이 업데이트해야 합니다.

![접점 위치 필터 및 FT/LC 필드를 사용하는 모든 필터](assets/release-notes-2.png)

![접점 위치 필터 및 FT/LC 필드를 사용하는 모든 필터](assets/release-notes-1.png)
...
* RefreshToken을 사용하는 OAuth의 오래된 메서드는 더 이상 사용되지 않습니다. ClientSecret 사용에 대한 Microsoft의 모범 사례를 준수하도록 자격 증명을 업데이트하는 방법은 [이 안내서](/help/oauth-with-azure-active-directory-for-dynamics-crm.md){target="_blank"}를 참조하십시오.

#### &quot;custom_properties&quot; 필드

Data Warehouse에서 &quot;custom_properties&quot; 필드는 고정 스키마에서 다루지 않는 추가 데이터 포인트를 위한 저장소 역할을 했습니다. JSON 형식으로 저장된 이 필드는 사용이 제한되고 SQL 쿼리와의 통합이 복잡하여 성능에 영향을 줄 수 있습니다. 이러한 요소들을 고려하여, 우리는 이 분야를 더 이상 사용하지 않기로 결정했다. 이 변경 사항은 주로 Azure 테이블 스토리지 내의 데이터 처리 계층과 데이터 웨어하우스로 내보낸 데이터에 영향을 줍니다.
...
[이 문서](/help/configuration-and-setup/salesforce-package-consolidation.md){target="_blank"}에는 기존 보고 패키지에서 보고서 및 대시보드를 다시 만드는 방법에 대한 지침이 포함되어 있습니다.
...
새 통합 패키지 설치 링크 [은(는) 여기에서 찾을 수 있습니다](https://login.salesforce.com/packaging/installPackage.apexp?p0=04t1P000000VY6Z){target="_blank"}
