...
>[!NOTE]
>[&#x200B; JavaScript을 배포하는 것 외에도  [!DNL Adobe Admin Console]](/help/marketo-measure-and-adobe/domain-management.md){target="_blank"}에서 [!DNL Marketo Measure]모든 적절한 도메인을 요청했는지 확인하십시오.
...
>[!TIP]
>스크립트가 [GDPR 규격](/help/security/ensuring-consent-for-gdpr-in-marketo-measure-js.md){target="_blank"}인지 확인하는 방법을 알아봅니다.

## Tag Management 시스템즈 {#tag-management-systems}

하드 코딩을 통해 [!DNL Marketo Measure] JavaScript을 추가할 수 없는 경우 다른 옵션은 [!DNL Marketo Measure]&#x200B;(GTM) 또는 Tealium과 같은 Tag Management 시스템을 사용하여 [!DNL Google Tag Manager] 스크립트를 추가하는 것입니다.
...
자세한 정보는 [여기에서 찾을 수 있음](/help/marketo-measure-tracking/adding-marketo-measure-script-via-google-tag-manager.md){target="_blank"}.
...
[!DNL Marketo Measure]이(가) 기여도 분석에 적합하지 않으므로 추적하지 말아야 할 양식이 있습니까? 등록 취소 양식, 고객 로그인 등 이 경우 이 문서[의 제외 코드 &#x200B;](/help/marketo-measure-tracking/excluding-marketo-measure-from-specific-forms.md){target="_blank"}을(를) 각 양식에 추가합니다.
...
JavaScript 설정 중에 [!DNL Web Application Firewall (WAF)] 경고가 트리거되면 사용자는 아래 예와 같이 해당 WAF 규칙 또는 쿠키 허용 목록을 비활성화할 수 있습니다.

![Marketo Measure 스크립트에 대한 WAF 경고 프롬프트 예](assets/adding-marketo-measure-script-1.png)
...
* 해결 방법: 캐시된 데이터를 기반으로 양식 중 하나가 [!DNL Marketo Measure]에게 사용자를 보고하고 포기 방법을 논의하도록 해야 합니다. 일반적으로 [사용자 코드 보고](/help/marketo-measure-tracking/ajax-form-handling.md){target="_blank"}를 통해 해결할 수 있습니다.
...
* 해결 방법: 웹 개발 팀에서 이러한 [보고서 사용자 템플릿](/help/marketo-measure-tracking/ajax-form-handling.md){target="_blank"}을(를) 사용하여 필요한 코드를 추가할 수 있습니다.
