...
콘텐츠 관리 시스템에서는 [!DNL Marketo Measure]이(가) 양식 제출을 인식하기 위해 표준 스크립트 구현 이상의 추가 단계를 수행해야 할 수 있습니다. 아래 프로세스에서는 [!DNL Marketo Measure] 페이지에 [!DNL Sitecore] Javascript를 추가하는 방법에 대해 간략히 설명합니다.

Sitecore 페이지가 있는 사이트의 경우:

1. Sitecore에 로그인하고 웹 사이트로 이동합니다. [!UICONTROL Configuration] 항목 및 [!UICONTROL Home] 폴더와 동일한 수준에 있는 [!UICONTROL Metadata] 폴더를 찾습니다.
1. **[!UICONTROL +]** 폴더 옆의 [!UICONTROL Configuration]을(를) 클릭합니다.
1. **[!UICONTROL +]** 폴더 옆의 [!UICONTROL Tools]을(를) 클릭합니다.
