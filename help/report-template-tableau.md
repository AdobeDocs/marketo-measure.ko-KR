...
![기존 연결 데이터를 특정 Snowflake으로 업데이트해야 합니다](assets/marketo-tableau-7.png)

## 데이터 연결 {#data-connection}

Snowflake 인스턴스에 데이터 연결을 설정해야 합니다. 이를 위해서는 사용자 이름 및 암호와 함께 서버 이름이 필요합니다. 필요한 경우 이 정보를 찾고 암호를 다시 설정하는 위치에 대한 자세한 정보는 [여기](/help/data-warehouse-access-reader-account.md){target="_blank"}에 설명되어 있습니다.

![Snowflake 인스턴스에 데이터 연결을 설정해야 합니다.](assets/marketo-tableau-5.png)
...
[!DNL Tableau]은(는) 필터가 설정된 개별 테이블이 아니라 전체 쿼리에 데이터 원본 필터를 적용하므로 모델의 각 테이블에 대해 사용자 지정 SQL을 사용하도록 선택했습니다. 이렇게 하면 모델이 테이블 수준에서 삭제 및 중복 행을 필터링할 수 있습니다. 예를 들어, 데이터 소스 필터로 적용할 경우 session._deleted_date is null이 쿼리의 where 절에 추가되어 다음 쿼리가 생성됩니다.
...
![Tableau의 데이터에 몇 가지 변형이 적용되었습니다](assets/marketo-tableau-1.png)

![Tableau의 데이터에 몇 가지 변형이 적용되었습니다](assets/marketo-tableau-2.png)
...
![Snowflake의 두 테이블에 있는 위치가 몇 개 있습니다](assets/marketo-tableau-3.png)
...
[![전체 크기 버전을 보려면 아래 이미지를 클릭하십시오.](assets/marketo-tableau-8.png)](assets/tableau-model-1.png){target="_blank"}
...
![비용과 접점 데이터 모두 채널 및 캠페인 차원을 공유합니다. 그러나 Tableau](assets/marketo-tableau-10.png)
...
![기업 통화 값을 선택한 통화 값으로 변환](assets/marketo-tableau-11.png)
...
![이 모델의 통화 전환 측정값은 1.0](assets/marketo-tableau-12.png) 값으로 대체됩니다.

## 데이터 정의 {#data-definitions}

매개 변수, 사용자 지정 열 및 측정값에 대한 정의가 [!DNL Tableau model]에 추가되었습니다.

![매개 변수, 사용자 지정](assets/marketo-tableau-15.png)에 대한 정의가 타블로 모델에 추가되었습니다.

[!DNL Snowflake]에서 직접 가져온 열의 정의를 보려면 [데이터 웨어하우스 설명서](/help/data-warehouse-schema.md){target="_blank"}를 참조하세요.
...
[!DNL Discover]의 [!UICONTROL Engagement Path] 보고서와 템플릿 간의 직접적인 비교는 없습니다. [!DNL Discover]의 보고서는 터치포인트에서 모델링되는 반면 템플릿의 보고서는 속성 터치포인트에서 모델링됩니다. 템플릿은 모든 터치포인트 데이터를 표시하는 대신 기회와 관련 터치포인트에만 중점을 둡니다.
