...
기능을 켜든 끄든 간에 발생한 모든 레코드 수준 오류가 `ExportErrors` 테이블에 기록되고 `CRMExport_ExportError` 작업은 매일 이러한 레코드를 자동으로 다시 내보냅니다. 이렇게 하면 개발자의 개입 없이 자동으로 수행되므로 재내보내기를 시작하기 위해 지원 요청이 필요하지 않습니다.
