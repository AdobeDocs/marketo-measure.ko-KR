---
description: 내보내기 권한 및 배치 설정을 포함하여 Marketo Measure이 Salesforce과 데이터를 교환하는 방법에 대한 개요
...
![Marketo Measure Salesforce 패키지를 설치하지 않은 경우](assets/bizible-full-1.png)
...
![기본적으로 Marketo Measure은 API 크레딧당 각각 200개의 레코드를 내보냅니다](assets/bizible-taxonomy-1.png)
...
여기에는 [!DNL Salesforce]이(가) 상호 작용하는 [!DNL Marketo Measure] 표준 개체와 연결이 설정되고 [!DNL Marketo Measure] 패키지가 설치되면 이 개체에 추가하는 사용자 지정 필드가 나열됩니다. 기본적으로 [!DNL Marketo Measure]은(는) 표준 [!DNL Salesforce] 개체 필드에 쓰지 않습니다.
...
## [!DNL Marketo Measure]의 사용자 지정 개체 [!DNL Salesforce]개  {#marketo-measure-custom-objects-in-salesforce}
