...
다음은 방법을 이해할 수 있는 부메랑 단계 시나리오의 몇 가지 예입니다 [!DNL Marketo Measure] 는 각 상황에서 터치포인트를 만듭니다.
...
이 시나리오에서 고객은 **MQL** 및 **SQL** 부메랑으로 추적된 단계. 각 부메랑 접점 위치는 단계 및 발생하는 번호(MQL-01, SQL-01, MQL-02)로 레이블이 지정됩니다. 해당 단계에 대한 마지막 부메랑 접점은 접점 위치에 &quot;(마지막)&quot;이 있습니다.
...
MQL 터치포인트 위치는 숫자로 레이블이 지정되지 않습니다. 부메랑 터치포인트로 추적하도록 선택하지 않았기 때문이다. 사용자 지정 모델에 포함되어 있지만 부메랑으로 추적되지 않는 단계에 대한 터치포인트를 만들 때, [!DNL Marketo Measure] 는 해당 단계에서 마지막 발생 횟수를 가져옵니다.

SAL은, [!DNL Marketo Measure] 에서는 이 스테이지의 처음 두 항목을 무시합니다. [!DNL Marketo Measure] 에 대한 SAL 터치포인트만 만듭니다. _마지막_ 발생 횟수. 위의 예에서 이 작업은 OC 터치포인트 바로 전에 발생합니다.
...
이 시나리오에서 Lead 는 실제로 SAL 단계로 전환되지 않습니다. SAL 단계에 도달하기 전에 Contact 로 변환되며, 기본적으로 SAL 단계는 &quot;건너뜀&quot;입니다. 이런 상황에서는 [!DNL Marketo Measure] sal은 OC 접점과 함께 발생하고 SAL과 OC 위치가 모두 동일한 접점에 나타난다고 가정한다.
...
이 시나리오에서 고객은 **데모 및 협상 단계 전환** 다음에 있음 **연락처**. 각 부메랑 스테이지는 최대 2개의 터치포인트를 받을 수 있다. Contact 의 단계 전환과 Lead 의 단계 전환의 차이점은 Contact 단계 전환이 Opportunity 의 Boomerang 터치포인트로 나타날 수 있다는 것입니다. _이후_ OC의 접점입니다. Lead에서 Form 터치포인트로 표시되므로 발생하는 Stage 전환에는 적용되지 않습니다.
...
협상 단계로 넘어가는 두 번째 전환은 없습니다. Opportunity 는 즉시 Demo-02 (Last)에서 Close Won으로 이동합니다. 이 경우, [!DNL Marketo Measure] 마감된 원 터치포인트와의 협상 전환이 포함됩니다.
