...
description: 접점 필드 - [!DNL Marketo Measure] - 제품 설명서
title: 접점 필드
...
이전에는 고객이 [!DNL Marketo Measure] 또한 직접 태그 지정 통합이 없는 경우 고객 성공 팀은 올바른 UTM 형식을 활용하고 광고를 해결할 수 있도록 고객에게 랜딩 페이지에 적절하게 태그를 지정하는 방법을 교육합니다. 이러한 고객 중 일부는 UTM을 사용하지 않고 자체 태그 지정 매개 변수를 사용하므로, 다음과 같은 새로운 태그 지정 구조를 사용하여 모든 광고 네트워크에서 모든 랜딩 페이지를 편집하는 데 시간이 많이 걸릴 수 있습니다. [!DNL Marketo Measure] 강제. 이제 태그 지정 구조에 적응하기 위해 규칙 정의와 함께 매핑할 수 있는 사용자 지정 매개 변수를 허용합니다. 목표는 고객이 사용자 지정 추적 매개 변수를 사용하는 데 적응하여 URL 구조를 변경할 필요가 없도록 하는 것입니다.
...
연락처 campaign_source__c에는 고객이 보고 목적으로 터치포인트에 놓을 사용자 정의 필드가 있습니다. 규칙을 정의하여 &quot;캠페인 소스&quot;라는 계산된 필드를 만들고 해당 필드에 값을 드롭할 수 있습니다.

목표: 보다 쉬운 보고를 위해 사용자 지정 필드의 값을 사용하여 터치포인트 오브젝트에 넣습니다.

* 계산된 필드를 만들고 &quot;Campaign Source&quot; 레이블 지정
...
**목표:** &quot;Discount Code&quot;라는 사용자 정의 필드를 만들고 &quot;5OFF&quot; 또는 &quot;25OFF&quot; 값을 드롭합니다. 어떤 값이 전달되든.
...
**목표:** 계산된 필드를 만들고 cid 매개 변수의 값을 사용하여 &quot;Adobe Campaign Id&quot;로 레이블을 지정합니다.
...
이제 를 추출하고 필드에 매핑하는 방법에 대해 알아보았으므로, 먼저 매개 변수에서 값을 추출한 다음 좀 더 의미가 있는 친숙한 이름으로 매핑하기 위해 이러한 작업을 결합해 보겠습니다. 이제 이 랜딩 페이지로 시작하겠습니다. `https://www.adobe.com/blog/marketing-revenue-reporting-overview?BZ=04-01-09-03-10`.

**목표:** 첫 번째 숫자가 영역에 매핑되고, 두 번째 숫자가 제품에 매핑되고, 세 번째 숫자가 이니셔티브에 매핑되고, 네 번째 숫자가 페르소나에 매핑되고, 다섯 번째 숫자가 미디어 플랫폼에 매핑되는 계산된 필드를 여러 개 만듭니다. 그런 다음 숫자 값을 &quot;친숙한 이름&quot;에 매핑합니다.

* 계산된 필드를 만들고 &quot;지역&quot;으로 레이블 지정
* Touchpoint.Session.LandingPage 필드 검색으로 시작하여 규칙을 정의합니다.
* 연산자 사용[!UICONTROL extracts]&quot;매개 변수에서 값을 추출해야 하므로
...
* 나머지 4자리 숫자 세트에 대해 추출과 매핑을 반복합니다
   ...
   * 01을 추출하려면 값을 &quot;BZ=\d{2}&quot;로 정의합니다.**(\d{2})**-\d{2}-\d{2}-\d{2}&quot;
   * 09를 추출하려면 값을 &quot;BZ=\d{2}-\d{2}-로 정의합니다.**(\d{2})**-\d{2}-\d{2}&quot;
   * 03을 추출하려면 값을 &quot;BZ=\d{2}-\d{2}-\d{2}-로 정의합니다.**(\d{2})**-\d{2}&quot;
   * 10을 추출하려면 값을 &quot;BZ=\d{2}-\d{2}-\d{2}-\d{2}-으로 정의합니다.**(\d{2})**&quot;
...
**만들 수 있는 최대 터치포인트 필드 수가 있습니까?**

필드는 100개로 제한됩니다.

**방금 만든 새 터치포인트 필드가 선택 목록에 표시되지 않습니다. 어디 있어?**
...
**단일 랜딩 페이지에서 여러 매개 변수를 추출하는 데 어떤 표현식을 사용해야 합니까?**

추출 예 #4와 마찬가지로 각 매개 변수를 추출하려면 여러 필드를 만들어야 합니다. 따라서 5개의 서로 다른 값이 있는 경우 5개의 터치포인트 필드를 만들어 각 값을 추출하게 됩니다.

**내 새 필드를 볼 수 없는 이유 [!DNL Marketo Measure] 스키마?**

의 새 필드를 표시하는 데 추가 작업이 필요합니다. [!DNL Marketo Measure] Data Warehouse 스키마. 현재 설정 및 구성을 통해 필드가 노출되므로 터치포인트 필드를 사용하여 세그먼트를 작성하거나 터치포인트 삭제 규칙을 만들 수 있습니다.
