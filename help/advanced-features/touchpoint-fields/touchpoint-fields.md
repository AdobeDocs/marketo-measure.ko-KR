...
계산된 필드 사용 옵션이 있는 ![터치포인트 필드 설정 페이지](assets/one.png)
...
![Campaign Source 필드 추출을 위한 계산된 필드 구성](assets/two.png)
...
![URL에서 프로모션 매개 변수를 추출하는 할인 코드 필드 구성](assets/three.png)
...
![6자리 cid 매개 변수를 추출하는 Adobe Campaign ID 필드](assets/four.png)
...
![국가 및 캠페인 ID 매개 변수를 추출하는 여러 계산된 필드](assets/five.png)
...
![홍보 범주에 캠페인을 매핑하는 이니셔티브 필드](assets/six.png)
...
![지역 이름에 숫자 코드가 있는 지역 이름 매핑 테이블](assets/seven.png)
...
![세그먼트와 등급 필드를 결합하는 구성 연결](assets/eight.png)
...
![사용 가능한 터치포인트 필드를 표시하는 세그먼트 만들기 인터페이스](assets/nine.png)
...
![중괄호 필드 참조가 있는 동적 세그먼트 구성](assets/ten.png)

규칙은 동일한 접점 필드를 참조하고 &quot;null과 같지 않음&quot;인 값을 검색합니다.

![Null 조건과 같지 않은 세그먼트 규칙](assets/eleven.png)
...
![올바른 식을 녹색으로 표시하는 Regex 유효성 검사 도구](assets/twelve.png)

![빨간색으로 잘못된 식을 표시하는 Regex 유효성 검사 도구](assets/thirteen.png)
