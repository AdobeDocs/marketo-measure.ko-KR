...
![단일 이미지 광고, 비디오 광고 및 리드 세대 Forms 지원을 통한 회전 메뉴 옵션을 보여 주는 LinkedIn 광고 형식 유형](assets/one.png)
...
![자동 태그 지정 프로세스 전에 연결된 공유와 함께 4개의 크리에이티브를 보여 주는 다이어그램](assets/two.png)
...
![자동 태그 지정 및 보관 프로세스 후에 만들어진 고유 공유로 새 크리에이티브를 보여 주는 다이어그램](assets/three.png)
...
![레퍼러 열이 있는 LinkedIn 유료 채널 정의를 표시하는 마케팅 채널 규칙 구성](assets/four.png)
...
![태그가 지정된 설명에 대상 URL과 단축된 URL을 모두 표시하는 LinkedIn 광고](assets/five.png)
...
![공유 ID가 URL에 표시되는 새 탭에서 열린 광고를 표시하는 LinkedIn Campaign 관리자](assets/six.png)
