...
![Marketo Measure 템플릿에 대한 Power BI 연결 대화 상자](assets/marketo-measure-report-template-power-bi-1.png)

특정 서버, 웨어하우스 및 스키마 정보는 [!DNL Marketo Measure] 정보 페이지의 [!DNL Data Warehouse] UI에서 찾을 수 있습니다. 이 페이지를 찾는 방법에 대한 지침은 [여기](/help/data-warehouse/data-warehouse-access-reader-account.md){target="_blank"}에 자세히 설명되어 있습니다.
...
파일을 열 때 입력한 매개 변수는 데이터 웨어하우스에서 테이블을 가져오는 기본 쿼리를 구성하는 데 사용됩니다. [!DNL Snowflake] 인스턴스에 대한 데이터 연결을 설정해야 합니다. 이 경우 사용자 이름 및 암호와 함께 동일한 서버 및 웨어하우스 이름이 필요합니다. 필요한 경우 사용자 이름을 찾고 암호를 재설정하는 위치에 대한 자세한 정보는 [여기](/help/data-warehouse/data-warehouse-access-reader-account.md){target="_blank"}에 설명되어 있습니다.
...
![Power BI의 기본 쿼리 승인 대화 상자](assets/marketo-measure-report-template-power-bi-2.png)
...
![날짜 필터 매개 변수를 표시하는 쿼리 편집기](assets/marketo-measure-report-template-power-bi-3.png)
...
![적용된 변환 단계를 표시하는 파워 쿼리 편집기](assets/marketo-measure-report-template-power-bi-4.png)
...
![제거된 열 구성을 표시하는 열 선택 대화 상자](assets/marketo-measure-report-template-power-bi-5.png)
...
![열 이름 매핑을 표시하는 이름이 변경된 열 단계](assets/marketo-measure-report-template-power-bi-6.png)
...
![파워 쿼리의 세그먼트 이름 매핑 테이블](assets/marketo-measure-report-template-power-bi-7.png)
...
![대/소문자를 구분하는 ID 변환을 보여 주는 터치포인트 테이블](assets/marketo-measure-report-template-power-bi-8.png)

![대/소문자 구분에 대한 조인 ID 열이 있는 Campaign 테이블](assets/marketo-measure-report-template-power-bi-9.png)
...
![회사 전환율 열이 있는 영업 기회 테이블](assets/marketo-measure-report-template-power-bi-10.png)
...
![확장된 날짜 행이 있는 전환율 테이블](assets/marketo-measure-report-template-power-bi-11.png)

## 데이터 모델 {#data-model}

전체 크기 버전을 보려면 아래 이미지를 클릭하십시오.

[![테이블 관계를 보여 주는 Power BI 데이터 모델 다이어그램](assets/marketo-measure-report-template-power-bi-12.png)](/help/bi-report-templates/assets/power-bi-data-model.png){target="_blank"}
...
![DAX 계산 논리를 보여 주는 통화 변환 수식](assets/marketo-measure-report-template-power-bi-13.png)

## 데이터 정의 {#data-definitions}

테이블, 사용자 정의 열 및 측정값에 대한 Power BI 모델에 정의가 추가되었습니다.

![정의 설명을 표시하는 표 속성](assets/marketo-measure-report-template-power-bi-14.png)

![데이터 모델의 사용자 지정 열 정의](assets/marketo-measure-report-template-power-bi-15.png)

![DAX 수식을 사용한 측정값 정의](assets/marketo-measure-report-template-power-bi-16.png)

[!DNL Snowflake]에서 직접 가져오는 열에 대한 정의를 보려면 [데이터 웨어하우스 설명서](/help/data-warehouse/data-warehouse-schema.md){target="_blank"}를 참조하세요.
