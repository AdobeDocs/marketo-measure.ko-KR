...
Snowflake 인스턴스에 데이터 연결을 설정해야 합니다. 이를 위해서는 사용자 이름 및 암호와 함께 서버 이름이 필요합니다. 필요한 경우 이 정보를 찾고 암호를 다시 설정하는 위치에 대한 자세한 정보는 [여기](/help/marketo-measure-data-warehouse/data-warehouse-access-reader-account.md){target="_blank"}에 설명되어 있습니다.
...
[!DNL Snowflake]에서 직접 가져온 열의 정의를 보려면 [데이터 웨어하우스 설명서](/help/marketo-measure-data-warehouse/data-warehouse-schema.md){target="_blank"}를 참조하세요.
