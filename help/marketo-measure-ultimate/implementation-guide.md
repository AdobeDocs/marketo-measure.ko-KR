...
## 표준 계층에서 Ultimate을 사용할 때의 주요 차이점 {#main-differences-when-using-ultimate-over-standard-tiers}

AEP을 통해 B2B 데이터 가져오기: 마케터는 AEP을 통해 B2B 데이터(예: 계정, 영업 기회, 연락처, 리드, 캠페인, 캠페인 멤버, 활동)를 가져와야 합니다. 거의 모든 데이터 소스 및 동일한 유형의 여러 데이터 소스에서 수집하여 속성을 위해 모든 데이터를 가져옵니다.
...
Ultimate에서 직접 CRM 및 Marketo Engage 연결을 더 이상 사용할 수 없습니다.

* Ultimate은 데이터를 CRM에 다시 푸시하지 않습니다. 고객은 Data Warehouse에서 데이터를 사용할 수 있습니다.
* 마케터는 직접 연결을 통해 Ad Platform 데이터를 계속 가져오고 Marketo Measure Javascript를 통해 웹 활동을 추적합니다.

Ultimate 사용자는 AEP에서 프로비저닝됩니다. 이미 AEP이 있는 경우 새 인스턴스를 다시 프로비저닝하지 않습니다.

* 제공된 AEP 버전에는 모든 소스 커넥터, 스키마 데이터 모델링, 데이터 세트, Ad Hoc Query Service 및 Marketo Measure 전용 대상이 포함됩니다.

[Marketo Measure Ultimate](/help/marketo-measure-ultimate/overview.md){target="_blank"}에 대해 자세히 알아보세요.

## 스키마 및 데이터 세트 {#schemas-and-datasets}

>[!NOTE]
>스키마, 클래스 및 필드 그룹에 대한 개요는 [스키마 빌딩 블록](https://experienceleague.adobe.com/docs/experience-platform/xdm/schema/composition.html?lang=ko#building-blocks-of-a-schema){target="_blank"}을 확인하십시오.
...
![클래스 및 필드 그룹 관계를 보여 주는 스키마 구조 다이어그램](assets/marketo-measure-ultimate-implementation-guide-1.png)

[데이터 세트 개요](https://experienceleague.adobe.com/docs/experience-platform/catalog/datasets/overview.html?lang=ko){target="_blank"}: AEP에 성공적으로 수집된 모든 데이터는 데이터 세트로 Data Lake 내에 보관됩니다. 데이터 세트는 스키마(열) 및 필드(행)를 포함하는 데이터 수집을 위한 저장소 및 관리 구조입니다. 

## 스키마 만들기 {#creating-a-schema}

10개의 표준 B2B 스키마를 만들려면 자동 생성 유틸리티를 사용하는 것이 좋습니다.

* [&#x200B; 유틸리티를 다운로드하고 설정하는 단계는 &#x200B;](https://experienceleague.adobe.com/docs/experience-platform/sources/connectors/adobe-applications/marketo/marketo-namespaces.html?lang=ko#set-up-b2b-namespaces-and-schema-auto-generation-utility){target="_blank"}에서 찾을 수 있습니다.

_&#x200B;**CDP 권한**&#x200B;_&#x200B;이 있는 사용자의 경우: 소스 페이지로 이동하여 스키마를 만드십시오.

* 소스에서 데이터 추가 > 템플릿 사용을 선택합니다.

![데이터 추가 및 템플릿 사용 옵션이 있는 소스 페이지](assets/marketo-measure-ultimate-implementation-guide-2.png)

* 계정 및 모든 B2B 템플릿을 선택하여 10개의 표준 B2B 스키마를 만듭니다.

![계정 구성에 대한 B2B 스키마 템플릿을 표시하는 템플릿 선택](assets/marketo-measure-ultimate-implementation-guide-3.png)
...
1. Source의 필드를 스키마에 매핑합니다.
   ...
   >[!NOTE]
   >[Marketo Measure Ultimate 데이터 무결성 요구 사항](/help/marketo-measure-ultimate/data-integrity-requirement.md){target="_blank"}
...
![쿼리 인터페이스를 표시하는 데이터 관리의 쿼리 탭](assets/marketo-measure-ultimate-implementation-guide-4.png)
...
> AEP에서 전체 대상을 삭제하여 Measure에서 데이터를 삭제합니다. 비활성화하면 새 데이터 내보내기가 중지되고 이전 데이터가 유지됩니다.
...
![여러 원본에서 단계 구성을 표시하는 단계 매핑 인터페이스](assets/marketo-measure-ultimate-implementation-guide-5.png)
